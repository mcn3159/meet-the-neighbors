        ...
        if args.subcommand == "extract_neighbors":
            ...
            if not args.plt_from_saved:
                ...
                if (not os.path.isfile(f"{args.out}genomesDB") and args.resume) or (not args.resume):
                    # i think these resume commands are fucked
            ...
            ,shell=True,check=True)
                ...
                if args.glm:
                    uniq_neighborhoods_d = {query:class_objs[query].get_neighborhood_names() for query in class_objs}
                    print(uniq_neighborhoods_d)
                    db.map(glm.get_glm_input,query=db.from_sequence(uniq_neighborhoods_d.keys()),uniq_neighborhoods_d=uniq_neighborhoods_d,out_dir=args.out)
