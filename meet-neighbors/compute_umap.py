...
def unpack_embeddings(glm_out_dir,glm_in_dir,mmseqs_clust): # lowkey don't need mmseqs_clust but it's here just to make sure things are working
    ...
    for vf in tqdm(glm_vf_fldrs_o):
        ...
        glm_batch = np.array(glm_batch)[input_tsv['glm_indicies'].values] #subset gLM batch for indicies of VF centers
        ...
        for i,embed in enumerate(glm_batch): # there should be an embedding for each row in input_tsv or for each VF center
            glm_res_d_vals_predf[vf+"!!!"+str(i)] = np.append(embed[1],input_tsv.iloc[0].loc[['neighborhood_name','vf_name','vfid','vf_subcategory','species','genus']].values)
...
def get_glm_umap_df(glm_res_d_vals_predf): # format dictionary to an embedding df for uMAP plotting
    # currently only works in virulent mode
